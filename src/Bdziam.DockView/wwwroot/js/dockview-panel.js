...
const updateTitle = panel => {
    ...
    const titleElement = contents.find(i => i.classList.contains('b-dockview-item-title'));
    if (titleElement) {
        tabEle.replaceChild(titleElement, panel.view.tab._content);
    }
    else {
        const titleBarElement = contents.find(i => i.classList.contains('b-dockview-item-title-icon'));
        if (titleBarElement) {
...
const findContentFromPanels = (panels, content) => {
    return panels.find(p => {
        ...
        // Check if both panels have params and keys, and they match
        if (p.params?.key && content.params?.key && p.params.key === content.params.key) {
            return true;
        }
        
        // Check if ids match
        if (p.id && content.id && p.id === content.id) {
            return true;
        }
        
        // Check if titles match
        if (p.title && content.title && p.title === content.title) {
            return true;
        }

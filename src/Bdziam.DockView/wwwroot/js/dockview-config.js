...
const removePanel = (branch, panel, parent) => {
    if (!branch || !panel) return;
    
    if (branch.type == 'leaf') {
        if (branch.data && branch.data.views && branch.data.views.length > 0) {
            branch.data.views = branch.data.views.filter(id => id != panel.id)
            if (branch.data.views.length == 0) {
                parent && parent.data && (parent.data = parent.data.filter(child => child.data && child.data.id != branch.data.id))
            }
        }
    }
    else if (branch.type == 'branch') {
        if (branch.data && Array.isArray(branch.data)) {
            ...
            if (branch.data.length == 0) {
                parent && parent.data && (parent.data = parent.data.filter(b => !(b.type == 'branch' && b.data && b.data.length == 0)))
            }
        }
    }
}
...
const getConfigFromContent = options => {
    ...
    // Check if options.content exists and has at least one element
    if (!options || !options.content || !Array.isArray(options.content) || options.content.length === 0) {
        console.warn('getConfigFromContent: Invalid or empty content array');
        return fixObject({
            activeGroup: '1',
            grid: { width, height, orientation: 'HORIZONTAL', root: null },
            panels: {}
        });
    }
    
    // Check if the first content item exists and has a type property
    const firstContent = options.content[0];
    if (!firstContent || typeof firstContent.type === 'undefined') {
        console.warn('getConfigFromContent: First content item is missing or has no type property');
        return fixObject({
            activeGroup: '1',
            grid: { width, height, orientation: 'HORIZONTAL', root: null },
            panels: {}
    ...
    const panels = {}, rootType = firstContent.type
    const orientation = rootType === 'column' ? 'VERTICAL' : 'HORIZONTAL';
    const root = getTree(firstContent, { width, height, orientation }, options, panels, getGroupId, options)
    return fixObject({
        activeGroup: '1',
...
const getTree = (contentItem, { width, height, orientation }, parent, panels, getGroupId, options) => {
    // Check if contentItem exists and has required properties
    if (!contentItem) {
        console.warn('getTree: contentItem is null/undefined');
        return null;
    }
    
    // Check if parent exists and has content array
    if (!parent || !parent.content || !Array.isArray(parent.content)) {
        console.warn('getTree: parent is missing or has invalid content array');
        return null;
    }
    ...
    let size;
    const hasSizeList = parent.content.filter(item => item && (item.width || item.height))
    ...
    if (contentItem.type === 'row' || contentItem.type === 'column') {
        ...
        // Check if contentItem.content exists and is an array before mapping
        if (contentItem.content && Array.isArray(contentItem.content)) {
            ...
            console.warn('getTree: contentItem.content is missing or not an array for row/column type');
            obj.data = [];
        }
    }

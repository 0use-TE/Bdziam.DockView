...
// Function to ensure CSS files are loaded
const ensureCssLoaded = () => {
    // Check if CSS files are already loaded
    const cssFiles = [
        '_content/Bdziam.DockView/css/bdziam-dockview.css',
        '_content/Bdziam.DockView/css/dockview.css'
    ];
    ...
    cssFiles.forEach(cssFile => {
        ...
        for (let i = 0; i < existingLinks.length; i++) {
            if (existingLinks[i].href.includes(cssFile)) {
                isLoaded = true;
                break;
            }
        }
        ...
        const handler = setTimeout(() => {
            ...
            dockview.floatingGroups.forEach(fg => {
                ...
                if (floatType == 'drawer') {
                    ...
                    const autoHideBtn = fg.group.header.rightActionsContainer.querySelector('.b-dockview-control-icon-autohide')
                    if (autoHideBtn) {
            ...
            dockview.element.querySelector('&>.dv-dockview>.dv-branch-node').addEventListener('click', function (e) {
                ...
                this.closest('.b-dockview').querySelectorAll('&>.b-dockview-aside>.b-dockview-aside-button').forEach(item => {
                    item.classList.remove('active')
                })
            })
...
const resizeObserverHandle = (observerList, dockview) => {
    ...
    if (dockview._processingResize) return;
    
    dockview._processingResize = true;
    try {
        observerList.forEach(({ target }) => {
            setWidth(target, dockview)
        })
    } finally {
        // Use a timeout to reset the flag after DOM changes have settled
        setTimeout(() => {
            dockview._processingResize = false;
        }, 0);
    }
}

...
const onAddGroup = (group) => {
  Object.defineProperties(group, {
    type: {
      get() {
        return this.model.location.type;
      },
    },
    params: {
      get() {
        return JSON.parse(JSON.stringify(group.activePanel?.params || {}));
      },
    },
  });
...
const addPanelWidthGroupId = (dockview, panel, index) => {
  ...
  if (!group) {
    ...
    let floatingGroupRect = rect || {
      width,
      height: packup?.isPackup ? packup.height : height,
      position: { left, top },
    };
    if (floatType == "drawer") {
      floatingGroupRect = {
    ...
    const overlay = dockview.floatingGroups.find(
      (fg) => fg.group.id == group.id,
    ).overlay;
    ...
    const autoHideBtn = group.header.rightActionsContainer.querySelector(
      ".b-dockview-control-icon-autohide",
    );
    if (autoHideBtn) {
...
const addPanelWidthCreatGroup = (dockview, panel, panels) => {
  ...
  let brothers = panels.filter(
    (p) => p.params.parentId == panel.params.parentId && p.id != panel.id,
  );
  let group, direction;
  if (brothers.length > 0 && brothers[0].params.parentType == "group") {
    group = dockview.groups.find((g) =>
      findContentFromPanels(g.panels, brothers[0]),
    );
  } else {
    let targetPanel;
    for (let i = 0, len = panels.length; i < len; i++) {
      if (panels[i]?.id === panel.id) {
        if (i == len - 1) {
          ...
          group = dockview.groups.find((g) =>
            findContentFromPanels(g.panels, targetPanel),
          );
          direction =
            getOrientation(dockview.gridview.root, group) === "VERTICAL"
              ? "below"
              : "right";
          break;
        } else {
...
const getOrientation = function (child, group) {
  // Check if child and group are valid objects
  if (!child || !group) {
    console.warn('getOrientation: child or group is null/undefined');
    return false;
  }
  
  if (child.children && Array.isArray(child.children)) {
    let targetGroup = child.children.find(
      (item) => item && !item.children && item.element && group.element && item.element === group.element,
    );
    if (targetGroup) {
...
const createGroupActions = (group, groupType) => {
  const actionContainer = group.header.element.querySelector(
  ...
  getIcons().forEach((item) => {
    if (item.name !== "bar") {
      ...
      if (icon) {
        ...
        if (icon.classList.contains("b-dockview-control-icon-dropdown")) {
          setTimeout(() => {
            if (
              group.model.location.type == "floating" &&
              group.panels.some((panel) => panel.renderer == "always")
            ) {
              observeDisplayChange(icon, group);
            } else {
              // Hide the dropdown when it's not needed
  ...
  setTimeout(() => {
    ...
    if (showUp(group) && getUpState(group)) {
      group.panels.forEach((panel) =>
        panel.view.content.element.classList.add("b-overflowHidden"),
      );
    }
  }, 0);
    ...
    mutations.forEach((mutation) => {
      if (mutation.attributeName == "class") {
        if (mutation.target.classList.contains("show")) {
          const currentPanelEle =
            group.activePanel.view.content.element.parentElement;
          ...
          const panelEleList = [
            ...group.element.querySelector("&>.dv-content-container").children,
          ].map((item) => {
...
const showPin = (dockview, group) => {
  ...
  return group.panels.every(
    (panel) =>
      panel.params.showPin === null || panel.params.showPin === undefined,
  )
    ? options.showPin
...
const getMaximizeState = (group) => {
  ...
  return type === "grid"
    ...
    : type === "floating"
      ? group.activePanel?.params.isMaximized
      : false;
};
...
const addActionEvent = (group) => {
  ...
  EventHandler.on(
    ...
    (e) => {
      ...
      const functionName = ele.getAttribute('data-function');
      
      if (functionName === "lock") {
        toggleLock(group, actionContainer, false);
        group.api.accessor._lockChanged.fire({
          title: group.panels.map((panel) => panel.title),
          isLock: false,
        });
      } else if (functionName === "lock_open") {
        toggleLock(group, actionContainer, true);
        group.api.accessor._lockChanged.fire({
          title: group.panels.map((panel) => panel.title),
          isLock: true,
        });
      } else if (functionName === "restore") {
        toggleFull(group, actionContainer, true);
      } else if (functionName === "full") {
        toggleFull(group, actionContainer, false);
      } else if (functionName === "dock") {
        dock(group);
        ...
        functionName === "close" &&
        ele.parentElement.classList.contains("dv-right-actions-container")
      ) {
        ...
        group.api.accessor.moveGroupOrPanel({
          from: {
            groupId: group.id,
            panelId: group.panels.find(
              (p) => p.view.tab.element.parentElement == targetTabEle,
            ).id,
          },
          to: {
            group,
...
const autoHide = (group) => {
  ...
  if (type == "grid") {
    ...
    const dockviewWidth =
      dockview.element.querySelector("&>.dv-dockview").offsetWidth;
    const isRight =
      (left + width == dockviewWidth && left > 0) || left > dockviewWidth / 2;
...
const createDrawerHandle = (floatingGroup, isRight) => {
  const className = isRight
    ? "b-resize-container-right"
    : "b-resize-container-left";
  floatingGroup.element.parentElement.classList.add(
    "dv-resize-container-drawer",
  );
  floatingGroup.element.parentElement.classList.add(className);
  floatingGroup.activePanel.view.content.element.parentElement.classList.add(
    "dv-render-overlay-float-drawer",
  );
...
const createDrawerBtn = (floatingGroup, isRight) => {
  ...
  const title =
    floatingGroup.activePanel?.title || floatingGroup.panels[0]?.title;
  btn.innerHTML = title;
  btn.setAttribute("groupid", dockview.id + "_" + floatingGroup.id);
  btn.classList.add("b-dockview-aside-button");
  ...
  btn.addEventListener("click", () => {
    ...
    dockview.floatingGroups.forEach((item) => {
      ...
      if (params.floatType == "drawer" && item.group != floatingGroup) {
        ...
        if (activePanel?.renderer == "always" && parentElement) {
          item.group.activePanel.view.content.element.parentElement.classList.remove(
            "active",
          );
        }
      }
    ...
    btn.parentElement.parentElement
      .querySelectorAll("&>.b-dockview-aside>.b-dockview-aside-button")
      .forEach((btnEle) => {
  ...
  const className = `b-dockview-aside-${isRight ? "right" : "left"}`;
  let btnWrapper = dvEleBox.querySelector(`&>.${className}`);
  if (!btnWrapper) {
    btnWrapper = document.createElement("aside");
    btnWrapper.className = className + " b-dockview-aside";
...
const removeDrawerBtn = (group) => {
  ...
  const btnList = parentEle?.querySelectorAll(
    `[groupid="${group.api.accessor.id}_${group.id}"]`,
  );
...
const setDrawerTitle = (group) => {
  ...
  const btnEle =
    group.api.accessor.element.parentElement.parentElement.querySelector(
      `.b-dockview-aside>[groupId="${groupId}"]`,
    );
  if (!btnEle) return;
...
const removeActionEvent = (group) => {
  ...
  EventHandler.off(actionContainer, "click", ".b-dockview-control-icon");
};
...
const toggleLock = (group, actionContainer, isLock) => {
  ...
  if (isLock) {
    actionContainer.classList.add("b-lock");
  } else {
    actionContainer.classList.remove("b-lock");
  }
...
const toggleFull = (group, actionContainer, maximize) => {
  ...
  maximize
    ? actionContainer.classList.remove("b-maximize")
    : actionContainer.classList.add("b-maximize");
  if (maximize) {
    group.element.parentElement.classList.remove("b-maximize");
  } else {
    group.element.parentElement.classList.add("b-maximize");
  }
};
...
const canFloat = (group) => {
  ...
  const dockview = group.api.accessor;
  const gridGroups = dockview.groups.filter(
    (g) => g.panels.length > 0 && g.model.location.type === "grid",
  );
...
const createFloatingGroup = (group, rect, groupType) => {
  ...
  const overlay = dockview.floatingGroups.find(
    (fg) => fg.group.id == floatingGroup.id,
  ).overlay;
...
const observeOverlayChange = (overlay, group) => {
  overlay.onDidChangeEnd((e) => {
    if (e.eventType == "drag" && group.getParams().floatType == "drawer") {
      ...
      parentEle.classList.remove("b-resize-container-right");
      parentEle.classList.remove("b-resize-container-left");
      group.activePanel.view.content.element.parentElement.classList.remove(
        "active",
      );
      group.activePanel.view.content.element.parentElement.classList.remove(
        "dv-render-overlay-float-drawer",
      ...
      group.header.rightActionsContainer.classList.remove("b-show-autohide");
      group.header.rightActionsContainer.classList.add("b-show-float");
      group.header.rightActionsContainer.querySelector(
        ".b-dockview-control-icon-down",
      ).style.display = "block";
    }
    saveConfig(group.api.accessor);
  });
...
const observeFloatingGroupLocationChange = (group) => {
  ...
  group.api.onDidLocationChange((e) => {
    if (e.location.type == "grid") {
      setTimeout(() => {
        let originalGroup = dockview.groups.find(
          (g) => g.id.split("_")[0] == group.id.split("_")[0],
        );
        if (originalGroup) {
          dockview.isClearing = true;
          dockview.removeGroup(originalGroup);
          dockview.isClearing = false;
          group.header.rightActionsContainer.classList.remove("b-float");
          saveConfig(dockview);
        }
      }, 0);
      removeDrawerBtn(group);
      group.header.rightActionsContainer.classList.add("b-pin");
    }
  });
};
...
const dock = (group, floatType) => {
  ...
  const dockview = group.api.accessor;
  const originGroup = dockview.groups.find(
    (g) => g.id.split("_")[0] == group.id.split("_")[0] && g.id != group.id,
  );
  if (!originGroup) return;
  dockview.setVisible(originGroup, true);
  const { drawer } = group.getParams();
  const inset = group.element.parentElement.style.inset
    .split(" ")
...
const down = (group, actionContainer) => {
  ...
  if (packup?.isPackup) {
    ...
    if (top == "auto") {
      parentEle.style.bottom =
        parseFloat(bottom) - (packup.height - tabHeight) + "px";
    }
    actionContainer.classList.add("b-up");
    group.panels.forEach((panel) =>
      panel.view.content.element.classList.remove("b-overflowHidden"),
    );
  } else {
    ...
    if (top == "auto") {
      parentEle.style.bottom =
        parseFloat(bottom) + (parseFloat(height) - tabHeight) + "px";
    }
    actionContainer.classList.remove("b-up");
    group.panels.forEach((panel) =>
      panel.view.content.element.classList.add("b-overflowHidden"),
    );
  }
...
const floatingMaximize = (group) => {
  ...
  const inset = parentEle.style.inset
    .split(" ")
    .map((item) => (isNaN(parseFloat(item)) ? item : parseFloat(item)));
  const rect = {
    width: group.width + 2,
...
const setWidth = (observerList) => {
  observerList.forEach(({ target }) => {
    ...
    let dropdown = header.querySelector(
      ".dv-right-actions-container>.dropdown",
    );
    if (!dropdown) return;
    let dropMenu = dropdown.querySelector(".dropdown-menu");
    if (voidWidth === 0) {
      if (tabsContainer.children.length <= 1) return;
      const inactiveTabs = header.querySelectorAll(
        ".dv-tabs-container>.dv-inactive-tab",
      );
      ...
      let firstLi =
        dropMenu.querySelector("li:has(.dv-active-tab)") ||
        dropMenu.children[0];
      if (firstLi) {
        let firstTab = firstLi.querySelector(".dv-tab");
        if (
          voidWidth > firstLi.tabWidth ||
          tabsContainer.children.length == 0
        ) {
...
export {
  ...
  toggleLock,
  disposeGroup,
  observeFloatingGroupLocationChange,
  observeOverlayChange,
  createDrawerHandle,
  removeDrawerBtn,
  setDrawerTitle,
};
